...
function setupPlayers() {
    ...
    for (let i = 0; i < numPlayers; i++) {
        playerNamesDiv.innerHTML += `<input type="text" placeholder="Nome do jogador ${i + 1}" id="player-${i}" required><br>`;
    }
...
function setupImpostores() {
    ...
    if ((numImpostors+1) >= numPlayers) {
        alert('Tem de haver uma diferença de pelo menos 2 jogadores a mais do que os impostores.');
        return;
    }
...
function updateCurrentPlayer() {
    document.getElementById('current-player').innerText = `Vez de ${playerNames[currentPlayerIndex]}`;
}
